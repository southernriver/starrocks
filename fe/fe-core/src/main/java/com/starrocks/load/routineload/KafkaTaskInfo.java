...
public class KafkaTaskInfo extends RoutineLoadTaskInfo {
    ...
    public KafkaTaskInfo(long timeToExecuteMs, long timeoutMs, KafkaTaskInfo kafkaTaskInfo,
                         Map<Integer, Long> partitionIdToOffset) {
        super(UUID.randomUUID(), kafkaTaskInfo.getJobId(), kafkaTaskInfo.getTaskScheduleIntervalMs(),
                timeToExecuteMs, timeoutMs, kafkaTaskInfo.getBeId(), kafkaTaskInfo.getStatistics());
        this.partitionIdToOffset = partitionIdToOffset;
    }
    ...
    public boolean isProgressKeepUp(RoutineLoadProgress progress, Map<String, Long> consumeLagsRowNum) {
        ...
        for (Map.Entry<Integer, Long> entry : latestPartOffset.entrySet()) {
            ...
            if (consumedOffset != null) {
                consumeLagsRowNum.put(String.valueOf(part), max(0, latestOffset - 1 - consumedOffset));
