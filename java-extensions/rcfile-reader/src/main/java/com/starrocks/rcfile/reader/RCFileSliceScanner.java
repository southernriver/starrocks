...
import static com.starrocks.rcfile.reader.RCFileScannerUtils.HIVE_TYPE_MAPPING;

public class RCFileSliceScanner extends ConnectorScanner {
    private static final Logger LOG = LogManager.getLogger(RCFileSliceScanner.class);
    ...
    private static final String INPUT_FORMAT = "org.apache.hadoop.hive.ql.io.RCFileInputFormat";
    private static final String SERDE_LIB = "org.apache.hadoop.hive.serde2.columnar.ColumnarSerDe";

    public RCFileSliceScanner(int fetchSize, Map<String, String> params) {
    ...
    private void initReader(JobConf jobConf, Properties properties) throws Exception {
        ...
        reader = (RecordReader<LongWritable, BytesRefArrayWritable>) inputFormatClass
    ...
    public void close() throws IOException {
        try (ThreadContextClassLoader ignored = new ThreadContextClassLoader(classLoader)) {
            ...
            e.printStackTrace();
            throw new IOException("Failed to close the rcfile slice reader.", e);
        }
    }
            ...
            for (; numRows < getTableSize(); numRows++) {
                ...
                for (int i = 0; i < requiredFields.length; i++) {
                    ...
                    if (fieldData == null) {
                        ...
                        ColumnValue fieldValue = new RCFileColumnValue(fieldInspectors[i], fieldData);
