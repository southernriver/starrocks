...
import static com.starrocks.rcfile.reader.RCFileScannerUtils.TIMESTAMP_UNIT_MAPPING;

public class RCFileColumnValue implements ColumnValue {
    private final Object fieldData;
    private final ObjectInspector fieldInspector;

    RCFileColumnValue(ObjectInspector fieldInspector, Object fieldData) {
    ...
    public String getTimestamp(ColumnType.TypeValue type) {
        ...
        if (RCFileScannerUtils.isMaybeInt64Timestamp(type) && (fieldData instanceof LongWritable)) {
            long datetime = ((LongWritable) fieldData).get();
            TimeUnit timeUnit = TIMESTAMP_UNIT_MAPPING.get(type);
            LocalDateTime localDateTime = RCFileScannerUtils.getTimestamp(datetime, timeUnit, true);
            return RCFileScannerUtils.formatDateTime(localDateTime);
        } else {
            return inspectObject().toString();
        }
    }
    ...
    public void unpackArray(List<ColumnValue> values) {
        ...
        for (Object item : items) {
            RCFileColumnValue cv = null;
            if (item != null) {
                cv = new RCFileColumnValue(itemInspector, item);
            }
            values.add(cv);
        }
    }
    ...
    public void unpackMap(List<ColumnValue> keys, List<ColumnValue> values) {
        ...
        for (Map.Entry kv : inspector.getMap(fieldData).entrySet()) {
            RCFileColumnValue cv0 = null;
            RCFileColumnValue cv1 = null;
            if (kv.getKey() != null) {
                cv0 = new RCFileColumnValue(keyObjectInspector, kv.getKey());
            }
            if (kv.getValue() != null) {
                cv1 = new RCFileColumnValue(valueObjectInspector, kv.getValue());
            }
            keys.add(cv0);
            values.add(cv1);
        }
    ...
    public void unpackStruct(List<Integer> structFieldIndex, List<ColumnValue> values) {
        ...
        for (int i = 0; i < structFieldIndex.size(); i++) {
            ...
            if (idx != null) {
                ...
                if (o != null) {
                    cv = new RCFileColumnValue(sf.getFieldObjectInspector(), o);
                }
            }
            values.add(cv);
        }
