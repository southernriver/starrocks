...
public class ThiveStorageFormatFileSliceScannerFactory implements ScannerFactory {
    static ChildFirstClassLoader classLoader;

    static {
        String basePath = System.getenv("STARROCKS_HOME");
        File dir = new File(basePath + "/lib/thivestorageformat-reader-lib");
        URL[] jars = Arrays.stream(Objects.requireNonNull(dir.listFiles()))
                .map(f -> {
                    try {
                        ...
                        e.printStackTrace();
                        throw new RuntimeException("Cannot init thivestorageformat slice classloader.", e);
                    }
                }).toArray(URL[]::new);
    ...
    public Class getScannerClass() throws ClassNotFoundException {
        try {
            return ClassLoader.getSystemClassLoader()
                    .loadClass("com.starrocks.thivestorageformat.reader.ThiveStorageFormatFileSliceScanner");
