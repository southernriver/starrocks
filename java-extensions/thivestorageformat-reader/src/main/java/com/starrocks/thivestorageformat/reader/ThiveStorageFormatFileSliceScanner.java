...
import static com.starrocks.thivestorageformat.reader.ThiveStorageFormatFileScannerUtils.HIVE_TYPE_MAPPING;

public class ThiveStorageFormatFileSliceScanner extends ConnectorScanner {
    private static final Logger LOG = LogManager.getLogger(ThiveStorageFormatFileSliceScanner.class);
    ...
    private RecordReader<LongWritable, IRecord> reader;
    ...
    //StorageEngineClient.ColumnStorageInputFormat
    private static final String INPUT_FORMAT = "StorageEngineClient.FormatStorageInputFormat";
    private static final String SERDE_LIB = "StorageEngineClient.FormatStorageSerDe";

    public ThiveStorageFormatFileSliceScanner(int fetchSize, Map<String, String> params) {
        ...
        this.offset = Long.parseLong(params.get("offset"));
    ...
    private Properties makeProperties() {
        ...
        properties.setProperty("columns.types", String.join(",", types));
        properties.setProperty("serialization.lib", SERDE_LIB);
        return properties;
    }

    private void initReader(JobConf jobConf, Properties properties) throws Exception {
        Path path = new Path(dataFilePath);
        FileSplit fileSplit = new FileSplit(path, offset, length, (String[]) null);
        InputFormat<?, ?> inputFormatClass = createInputFormat(jobConf, INPUT_FORMAT);
        reader = (RecordReader<LongWritable, IRecord>) inputFormatClass
                .getRecordReader(fileSplit, jobConf, Reporter.NULL);

        deserializer = getDeserializer(jobConf, properties, SERDE_LIB);
    ...
    public void open() throws IOException {
        try (ThreadContextClassLoader ignored = new ThreadContextClassLoader(classLoader)) {
            ...
            close();
            e.printStackTrace();
            throw new IOException("Failed to open the rcfile slice reader.", e);
        }
    }
    ...
    public void close() throws IOException {
        try (ThreadContextClassLoader ignored = new ThreadContextClassLoader(classLoader)) {
            ...
            e.printStackTrace();
            throw new IOException("Failed to close the thive storageformat slice reader.", e);
        }
    }
    ...
    public int getNext() throws IOException {
        ...
        try (ThreadContextClassLoader ignored = new ThreadContextClassLoader(classLoader)) {
            ...
            IRecord value = reader.createValue();
            int numRows = 0;
            for (; numRows < getTableSize(); numRows++) {
                ...
                for (int i = 0; i < requiredFields.length; i++) {
                    ...
                    if (fieldData == null) {
                        ...
                        ColumnValue fieldValue = new ThiveStorageFormatFileColumnValue(fieldInspectors[i], fieldData);
            ...
            close();
            LOG.error("Failed to get the next off-heap table chunk of thive storageformat.", e);
            throw new IOException("Failed to get the next off-heap table chunk of thive storageformat.", e);
        }
    }
