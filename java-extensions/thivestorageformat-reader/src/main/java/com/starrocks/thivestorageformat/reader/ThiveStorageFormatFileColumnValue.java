...
import static com.starrocks.thivestorageformat.reader.ThiveStorageFormatFileScannerUtils.TIMESTAMP_UNIT_MAPPING;

public class ThiveStorageFormatFileColumnValue implements ColumnValue {
    private final Object fieldData;
    private final ObjectInspector fieldInspector;

    ThiveStorageFormatFileColumnValue(ObjectInspector fieldInspector, Object fieldData) {
    ...
    public String getTimestamp(ColumnType.TypeValue type) {
        ...
        if (ThiveStorageFormatFileScannerUtils.isMaybeInt64Timestamp(type) && (fieldData instanceof LongWritable)) {
            long datetime = ((LongWritable) fieldData).get();
            TimeUnit timeUnit = TIMESTAMP_UNIT_MAPPING.get(type);
            LocalDateTime localDateTime = ThiveStorageFormatFileScannerUtils.getTimestamp(datetime, timeUnit, true);
            return ThiveStorageFormatFileScannerUtils.formatDateTime(localDateTime);
        } else {
            return inspectObject().toString();
        }
    }
    ...
    public void unpackArray(List<ColumnValue> values) {
        ...
        for (Object item : items) {
            ThiveStorageFormatFileColumnValue cv = null;
            if (item != null) {
                cv = new ThiveStorageFormatFileColumnValue(itemInspector, item);
            }
            values.add(cv);
        }
    }
    ...
    public void unpackMap(List<ColumnValue> keys, List<ColumnValue> values) {
        ...
        for (Map.Entry kv : inspector.getMap(fieldData).entrySet()) {
            ThiveStorageFormatFileColumnValue cv0 = null;
            ThiveStorageFormatFileColumnValue cv1 = null;
            if (kv.getKey() != null) {
                cv0 = new ThiveStorageFormatFileColumnValue(keyObjectInspector, kv.getKey());
            }
            if (kv.getValue() != null) {
                cv1 = new ThiveStorageFormatFileColumnValue(valueObjectInspector, kv.getValue());
            }
            keys.add(cv0);
            values.add(cv1);
        }
    ...
    public void unpackStruct(List<Integer> structFieldIndex, List<ColumnValue> values) {
        ...
        for (int i = 0; i < structFieldIndex.size(); i++) {
            ...
            if (idx != null) {
                ...
                if (o != null) {
                    cv = new ThiveStorageFormatFileColumnValue(sf.getFieldObjectInspector(), o);
                }
            }
            values.add(cv);
        }
