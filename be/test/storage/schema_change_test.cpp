...
protected:
    ...
    void test_convert_to_varchar(LogicalType type, int type_size, T val, const std::string& expect_val) {
        ...
        src_datum.set<T>(val);
        Datum dst_datum;
        auto converter = vectorized::get_type_converter(type, TYPE_VARCHAR);
    ...
    void test_convert_from_varchar(LogicalType type, int type_size, std::string val, T expect_val) {
        ...
        Datum dst_datum;
        auto converter = vectorized::get_type_converter(TYPE_VARCHAR, type);
        std::unique_ptr<MemPool> mem_pool(new MemPool());
        Status st = converter->convert_datum(f.type().get(), src_datum, f2.type().get(), &dst_datum, mem_pool.get());
        ASSERT_TRUE(st.ok());
...
TEST_F(SchemaChangeTest, convert_float_to_double) {
    ...
    Datum dst_datum;
    auto converter = vectorized::get_type_converter(TYPE_FLOAT, TYPE_DOUBLE);
    std::unique_ptr<MemPool> mem_pool(new MemPool());
    Status st = converter->convert_datum(f.type().get(), src_datum, f2.type().get(), &dst_datum, mem_pool.get());
    ASSERT_TRUE(st.ok());
...
TEST_F(SchemaChangeTest, convert_datetime_to_date) {
    ...
    Datum dst_datum;
    auto converter = vectorized::get_type_converter(TYPE_DATETIME_V1, TYPE_DATE_V1);
...
TEST_F(SchemaChangeTest, convert_date_to_datetime) {
    ...
    Datum dst_datum;
    auto converter = vectorized::get_type_converter(TYPE_DATE_V1, TYPE_DATETIME_V1);
...
TEST_F(SchemaChangeTest, convert_int_to_date_v2) {
    ...
    Datum dst_datum;
    auto converter = vectorized::get_type_converter(TYPE_INT, TYPE_DATE);
...
TEST_F(SchemaChangeTest, convert_int_to_date) {
    ...
    src_datum.set_int32(20210928);
    Datum dst_datum;
    auto converter = vectorized::get_type_converter(TYPE_INT, TYPE_DATE_V1);
...
TEST_F(SchemaChangeTest, convert_int_to_bitmap) {
    ...
    auto converter = vectorized::get_materialized_converter(TYPE_INT, OLAP_MATERIALIZE_TYPE_BITMAP);
...
TEST_F(SchemaChangeTest, convert_varchar_to_hll) {
    ...
    auto converter = vectorized::get_materialized_converter(TYPE_VARCHAR, OLAP_MATERIALIZE_TYPE_HLL);
    Status st = converter->convert_materialized(src_col, dst_col, f.type().get());
    ASSERT_TRUE(st.ok());
...
TEST_F(SchemaChangeTest, convert_int_to_count) {
    ...
    auto converter = vectorized::get_materialized_converter(TYPE_INT, OLAP_MATERIALIZE_TYPE_COUNT);
    Status st = converter->convert_materialized(src_col, dst_col, f.type().get());
    ASSERT_TRUE(st.ok());
...
TEST_F(SchemaChangeTest, convert_varchar_to_json) {
    ...
    for (const auto& json_str : test_cases) {
        ...
        auto converter = vectorized::get_type_converter(TYPE_VARCHAR, TYPE_JSON);
...
TEST_F(SchemaChangeTest, convert_json_to_varchar) {
    ...
    auto converter = vectorized::get_type_converter(TYPE_JSON, TYPE_VARCHAR);
    auto mem_pool = std::make_unique<MemPool>();
...
TEST_F(SchemaChangeTest, lsc_context_schema_change_with_directing_v2) {
    CreateSrcTablet(1101);
    StorageEngine* engine = StorageEngine::instance();
    TCreateTabletReq create_tablet_req;
    SetCreateTabletReq(&create_tablet_req, 1102, TKeysType::DUP_KEYS);
    ...
    // pretend to add a additional column with light schema change request
    TabletSchemaSPtr base_tablet_schema = std::make_shared<TabletSchema>();
    base_tablet_schema->copy_from(base_tablet->tablet_schema());
    auto t_column = TColumn();
    t_column.__set_col_unique_id(4);
    t_column.__set_column_name("v3");
    t_column.__set_default_value("1");
    TColumnType t_column_type;
    t_column_type.__set_type(TPrimitiveType::INT);
    t_column.__set_column_type(t_column_type);
    base_tablet_schema->append_column(TabletColumn(t_column));
    ...
    read_params.skip_aggregation = false;
    read_params.chunk_size = config::vector_chunk_size;
    vectorized::Schema base_schema = ChunkHelper::convert_schema_to_format_v2(base_tablet_schema);
    auto* tablet_rowset_reader = new TabletReader(base_tablet, rowset->version(), base_schema, base_tablet_schema);
    ASSERT_TRUE(tablet_rowset_reader != nullptr);
    ASSERT_TRUE(tablet_rowset_reader->prepare().ok());
    ASSERT_TRUE(tablet_rowset_reader->open(read_params).ok());
    ...
    ASSERT_TRUE(
            _sc_procedure->process_v2(tablet_rowset_reader, rowset_writer.get(), new_tablet, base_tablet,
                                      rowset, base_tablet_schema).ok());
    delete tablet_rowset_reader;
    (void)StorageEngine::instance()->tablet_manager()->drop_tablet(1101);
    (void)StorageEngine::instance()->tablet_manager()->drop_tablet(1102);
}
