...
#ifdef __SSE4_1__
    uint8_t second {};
    /// vector filled `first` or `second` for determining leftmost position of the first symbol
...
#ifdef __SSE4_1__
            if (haystack + 1 + SSE2_WIDTH <= haystack_end && pageSafe(haystack)) {
                ...
                /// first and second character not present in 16 octets starting at `haystack`
                if (mask == 0) {
                    haystack += SSE2_WIDTH;
                    continue;
                }
