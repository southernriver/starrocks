        ...
        for (size_t i = 1; i < column_groups.size(); ++i) {
            ...
            if (_context.schema_change_sorting) {
                if (_context.tablet_schema->keys_type() == KeysType::DUP_KEYS ||
                    ...
                    chunk_itr = new_mask_merge_iterator(seg_iterators, mask_buffer.get());
                           ...
                           _context.tablet_schema->keys_type() == KeysType::AGG_KEYS) {
                    chunk_itr =
                            new_aggregate_iterator(new_mask_merge_iterator(seg_iterators, mask_buffer.get()), false);
                } else {
                    return Status::NotSupported(
            ...
            while (true) {
                ...
                if (st.is_end_of_file()) {
                    ...
                    ChunkHelper::padding_char_columns(char_field_indexes_, chunk_schema, _context.tablet_schema,
                                                      chunk_);
                    if (auto status = _vertical_rowset_writer->add_columns(*chunk_, column_groups[i], false);
                        !status.ok()) {
                        LOG(WARNING) << "writer add_columns error. tablet=" << _context.tablet_id << ", err=" << status;
                        return status;
                    }
