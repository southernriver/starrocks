...
SegmentWriter::SegmentWriter(std::unique_ptr<WritableFile> wfile, uint32_t segment_id,
                             const TabletSchemaCSPtr& tablet_schema, SegmentWriterOptions opts)
...
Status SegmentWriter::init(const std::vector<uint32_t>& column_indexes, bool has_key, SegmentFooterPB* footer) {
    ...
    for (uint32_t column_index : _column_indexes) {
        ...
        opts.need_zone_map = column.is_key() || (_tablet_schema->keys_type() == KeysType::DUP_KEYS &&
                                                 column.type() != LogicalType::TYPE_CHAR &&
                                                 column.type() != LogicalType::TYPE_VARCHAR &&
                                                 column.type() != LogicalType::TYPE_JSON &&
                                                 column.type() != LogicalType::TYPE_OBJECT &&
                                                 column.type() != LogicalType::TYPE_HLL &&
                                                 column.type() != LogicalType::TYPE_PERCENTILE);
