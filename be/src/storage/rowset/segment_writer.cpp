...
Status SegmentWriter::init(const std::vector<uint32_t>& column_indexes, bool has_key, SegmentFooterPB* footer) {
    ...
    for (uint32_t column_index : _column_indexes) {
        ...
        opts.need_zone_map = column.is_key() || (_tablet_schema->keys_type() == KeysType::DUP_KEYS &&
                                                 column.type() != LogicalType::LOGICAL_TYPE_CHAR &&
                                                 column.type() != LogicalType::LOGICAL_TYPE_VARCHAR &&
                                                 column.type() != LogicalType::LOGICAL_TYPE_JSON &&
                                                 column.type() != LogicalType::LOGICAL_TYPE_OBJECT &&
                                                 column.type() != LogicalType::LOGICAL_TYPE_HLL &&
                                                 column.type() != LogicalType::LOGICAL_TYPE_PERCENTILE);
