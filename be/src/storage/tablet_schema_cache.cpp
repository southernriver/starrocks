...
TabletSchemaSPtr TabletSchemaCache::insert(const std::string& key) {
    ...
    if (iter == _cache.end()) {
        ...
        TabletSchemaSPtr tablet_schema_ptr = TabletSchema::create(pb);
        _cache[key] = tablet_schema_ptr;
        StarRocksMetrics::instance()->tablet_schema_cache_count.increment(1);
        StarRocksMetrics::instance()->tablet_schema_cache_memory_bytes.increment(
                tablet_schema_ptr->mem_usage());
