...
Status NodeChannel::_filter_indexes_with_where_expr(vectorized::Chunk* input, const std::vector<uint32_t>& indexes,
                                                    std::vector<uint32_t>& filtered_indexes) {
    ...
    vectorized::Buffer<uint8_t> filter(size, 0);
    vectorized::ColumnViewer<TYPE_BOOLEAN> col(filter_col);
    for (size_t i = 0; i < size; ++i) {
...
Status NodeChannel::_send_request(bool eos, bool wait_all_sender_close) {
    ...
    for (int i = 0; i < request.requests_size(); i++) {
        ...
        if (UNLIKELY(eos)) {
            req->set_eos(true);
            if (wait_all_sender_close) req->set_wait_all_sender_close(true);
