...
public:
    Status send_chunk(const OlapTableSchemaParam* schema, const std::vector<vectorized::OlapTablePartition*>& partitions,
                      const std::vector<uint32_t>& tablet_indexes, const std::vector<uint16_t>& validate_select_idx,
                      std::unordered_map<int64_t, std::set<int64_t>>& index_id_partition_id, vectorized::Chunk* chunk) override;
};
