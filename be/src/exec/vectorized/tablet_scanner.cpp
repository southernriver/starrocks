...
Status TabletScanner::init(RuntimeState* runtime_state, const TabletScannerParams& params) {
    ...
    Schema child_schema = ChunkHelper::convert_schema_to_format_v2(_tablet_schema, _reader_columns);
    ...
    if (_reader_columns.size() == _scanner_columns.size()) {
        ...
        Schema output_schema = ChunkHelper::convert_schema_to_format_v2(_tablet_schema, _scanner_columns);
