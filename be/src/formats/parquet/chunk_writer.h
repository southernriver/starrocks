...
public:
    ChunkWriter(::parquet::RowGroupWriter* rg_writer, const std::vector<TypeDescriptor>& type_descs,
                ...
                const std::function<StatusOr<vectorized::ColumnPtr>(vectorized::Chunk*, size_t)>& eval_func);
...
private:
    ...
    std::function<StatusOr<vectorized::ColumnPtr>(vectorized::Chunk*, size_t)> _eval_func;
