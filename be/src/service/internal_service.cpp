...
void PInternalServiceImplBase<T>::get_column_ids_by_tablet_ids(google::protobuf::RpcController* controller,
    ...
    bool ret = _async_thread_pool.try_offer(
            [&]() { _get_column_ids_by_tablet_ids(controller, request, response, done, &latch); });
    if (!ret) {
...
void PInternalServiceImplBase<T>::_get_column_ids_by_tablet_ids(
        google::protobuf::RpcController* controller, const PFetchColIdsRequest* request, PFetchColIdsResponse* response,
        google::protobuf::Closure* done, GenericCountDownLatch<bthread::Mutex, bthread::ConditionVariable>* latch) {
